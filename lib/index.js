...
/** HTTP bridge prefix the client settings page fetches for its switch document. */
...
/** Servable cap for a `setConfig` request body. */
...
/**
 * HTTP routes for the client settings page (`/api/coding-discipline`). The
 * browser half has no process-transport RPC (`host.call` is a dynamic-package
 * builtin), so the settings document crosses as JSON over the webServer
 * bridge, matching how shipped profile bundles serve their settings.
 * @param syncSkill - re-register the runtime skill when switches change.
 * @returns webServer route registrations.
 */
function makeBridgeRoutes(syncSkill) {
  return [
    {
      kind: 'exact',
      path: `${BRIDGE_PREFIX}/getConfig`,
      handler: async (req, res) => {
        writeJson(res, 200, { ok: true, config: readConfig().config })
      },
    },
    {
      kind: 'exact',
      path: `${BRIDGE_PREFIX}/listModules`,
      handler: async (req, res) => {
        writeJson(res, 200, { ok: true, modules: listModules(readConfig().config) })
      },
    },
    {
      kind: 'exact',
      path: `${BRIDGE_PREFIX}/paths`,
      handler: async (req, res) => {
        writeJson(res, 200, { ok: true, config: configPath(), modules: modulesDir(), root: rootDir() })
      },
    },
    {
      kind: 'exact',
      path: `${BRIDGE_PREFIX}/setConfig`,
      handler: async (req, res) => {
...
export function apply(ctx) {
  ...
  // Settings-page bridge for the browser. In this DSH the client half of a
  // profile bundle has no process-transport RPC (`host.call`/`harness.handle`
  // are dynamic-package builtins), so the switch document crosses as JSON over
  // the webServer bridge — the same seam shipped bundles use for their
  // settings. The `harness` registrations below are retained for headless
  // telemetry / tests when that builtin is present.
  const registerBridge = (webCtx) => {
    webCtx.effect(function* () {
      const disposers = makeBridgeRoutes(syncSkill).map((route) => webCtx.webServer.register(route))
      yield () => {
    ...
    }, 'coding-discipline: settings bridge')
  }
  if (ctx.get('webServer') === undefined) {
    try {
      ctx.inject(['webServer'], registerBridge)
    } catch (error) {
      warn(`设置页桥接不可用（无 webServer）：${messageOf(error)}`)
    }
  } else {
    registerBridge(ctx)
  }
